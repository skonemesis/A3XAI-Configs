...
waitUntil {uiSleep 0.3; (!isNil "A3XAI_customSpawnsReady")};
...
call {
	if (_vehicleType isKindOf "Air") exitWith {
		...
		_maxCargoUnits = A3XAI_airCargoUnits;		// custom added by The Gray Jacket
	...
	if (_vehicleType isKindOf "LandVehicle") exitWith {
		...
		while {_keepLooking} do
		{
			...
			if ((count _vehiclePosition) > 1) then
			{
				...
				{
					diag_log format ["A3XAI Debug: Unable to find road position to spawn AI %1. Retrying in 15 seconds.",_vehicleType];
					uiSleep 15; //Couldnt find road, search again in 15 seconds.
				};
			};
		};
	};
...
if (count (nearestLocations [_vehiclePosition,[BLACKLIST_OBJECT_GENERAL,BLACKLIST_OBJECT_RANDOM],1500]) > 0) exitWith
{
	diag_log format ["A3XAI Debug: Blocked attempt to spawn %2 at %1. A spawn position is within a blacklisted area.",_vehiclePosition,_vehicleType];
};
