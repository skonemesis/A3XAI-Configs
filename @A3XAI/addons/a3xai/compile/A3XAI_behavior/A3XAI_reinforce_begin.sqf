...
_despawnPos = [_vehPos,AIR_REINFORCE_DESPAWN_DIST,(360),1] call A3XAI_SHK_pos;
...
_waypoint setWaypointType "SAD";
_waypoint setWaypointBehaviour "AWARE";
if (_vehicleArmed) then {_waypoint setWaypointCombatMode "RED";};
if (local _unitGroup) then {
	_unitGroup setCurrentWaypoint _waypoint;
} else {
	A3XAI_setCurrentWaypoint_PVC = _waypoint;
...
waitUntil {uiSleep 120; (((getPosATL _vehicle) distance2D _vehPos) > 6000) or {!(alive _vehicle)}};
