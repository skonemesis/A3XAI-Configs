...
if (_isInfantry) then {
	...
	if (_unitType in ["land","ugv"]) then 
	{
		...
		while {_keepLooking} do 
		{
			...
			{
				if (({if (_newPosEmpty in _x) exitWith {1}} count (nearestLocations [_newPosEmpty,[BLACKLIST_OBJECT_GENERAL,BLACKLIST_OBJECT_RANDOM],1500])) > 0) then
				{
					_keepLooking = true;
					diag_log format ["A3XAI Debug: Blocked attempt to relocate %2 at %1. A spawn position is within a blacklisted area.",_newPosEmpty,_vehicleType];
				} else
				{
					if (({isPlayer _x} count (_newPosEmpty nearEntities [[PLAYER_UNITS,"AllVehicles"], PLAYER_DISTANCE_WITH_LOS_ANTISTUCK]) isEqualTo 0) && {((_newPosEmpty nearObjects [PLOTPOLE_OBJECT,PLOTPOLE_RADIUS]) isEqualTo [])}) then
					{
